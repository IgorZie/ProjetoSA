...
if (isset($_GET['id']) && isset($_GET['action'])) {
    ...
    if ($_GET['action'] == "delete") {
        $queryDeletar = "DELETE FROM autor WHERE Id_Autor=$Id_Autor";

        if(!mysqli_query($conexao, $queryDeletar)){

            echo '<script type="text/javascript">';
            echo 'alert("Autor não pode ser excluído!");';
            echo 'window.location.href = "http://localhost/ProjetoSA/ListarLivros/listaAutores.php";';
            echo '</script>';
        }   
        else{
            echo '<script type="text/javascript">';
            echo 'alert("Autor deletado com sucesso!");';
            echo 'window.location.href = "http://localhost/ProjetoSA/ListarLivros/listaAutores.php";';
            echo '</script>';
        }
    }
...
$filtrar = "";
$queryFiltrar = "";
$ordenacao = ' ORDER BY Id_Autor ASC';
?>
...
<body>
    <header>
        ...
        <h1 style="margin-left: 250px;">Lista dos Autores</h1>
    ...
    if (isset($_POST['filtro'])) {
        ...
        if (isset($_POST['btn-filtro'])) {
            ...
            $queryFiltrar = " WHERE (Nome_Autor LIKE '%$filtrar%') GROUP BY Nome_Autor";
    ...
    $queryListar = 'SELECT * FROM autor' . $ordenacao . $queryFiltrar;


    echo '<br>';
    echo '<div class="container-fluid"><form action="./listaAutores.php" method="POST" id="form-filtro" name="form-filtro">';
    ...
    echo '<br> <thead>';
    echo '<tr><th scope="col">Id Autor</th>'
        . '<th scope="col">Nome Autor</th>'
        . '<th scope="col">Ações</th>'
        . '</tr> </thead> <tbody>';
    while ($linha_Autor = mysqli_fetch_array($resultadoListar)) {
        echo "<tr><td>$linha_Autor[Id_Autor]</td>"
            . "<td>$linha_Autor[Nome_Autor]</td>"
            . "<td><a href='listaAutores.php?id=" . $linha_Autor['Id_Autor'] . "&action=delete' class='btn-excluir' name='btn_delete' id='btn_delete_$linha_Autor[Id_Autor]'>Deletar</a></td>"
            . "</tr>";
    }
    echo '</tbody> </table>';
